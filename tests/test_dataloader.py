...
class TestDataLoader(unittest.TestCase):
    ...
    def test_mask_examples(self):
        ...
        masked_ds = masked_ds.create_tf_dataset_from_all_clients()
        masked_ds = masked_ds.filter(lambda x: x['is_masked_supervised']).batch(20)
        a = iter(masked_ds).next()
        print(a)
        print(a.shape)

        # a = tf.data.experimental.cardinality(masked_ds)
        # print(a)

        a = sum([1 for _ in iter(masked_ds)])
        print(a)

        # num_unmasked_examples = 0
        # for example in masked_ds:
